...
public class ColorPanel
    ...
    private Button createApplyButton( Composite comp ) {
        Button fab = toolkit.createFab();
        fab.setToolTipText( "Apply selection" );
        fab.addSelectionListener( new SelectionAdapter() {

            @Override
            public void widgetSelected( SelectionEvent ev ) {
                colorInfo.get().updateColor( getRGB() );
                PanelPath path = getSite().getPath();
                getContext().closePanel( path );
                toolkit.createSnackbar( Appearance.FadeIn, "Applied" );
            }
        } );
        return fab;
    }
