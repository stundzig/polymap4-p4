...
public abstract class StylePropertyEditor<SPV extends StylePropertyValue> {
     ...
     * @param spv
     */
    public static StylePropertyEditor[] forValue( Property<StylePropertyValue> prop, IAppContext context, IPanelSite site ) {
        ...
        for (Class<StylePropertyEditor> cl : availableEditors) {
            try {
                ...
                if (editor.init( prop )) {
                    ...
                    editor.site = site;
                    // FIXME, another solution to inject the Scope variables
                    new PanelContextInjector( editor, (DefaultAppContext)context ).run();
                    result.add( editor );
                }
            }
            catch (Exception e) {
